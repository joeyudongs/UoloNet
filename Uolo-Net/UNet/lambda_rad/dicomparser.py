        ...
        if 'PatientName' in self.ds:
            ...
            if isinstance(name, str):
                ...
                if hasattr(name, 'family_name'):
                    ...
                    patient['family_name'] = ''
                patient['name'] = patient['family_name'] + "," + patient['middle_name'] + "," + patient['given_name']

        if 'PatientID' in self.ds:
            #patientId= ((self.ds[(0x0010, 0x0020)]).repval).encode('UTF-8')
            patientId = self.ds.PatientID
            #if patientId.find(r'\x0'):
            #    patientId = patientId[1: patientId.find(r'\x0')].replace("\'", '').strip()
